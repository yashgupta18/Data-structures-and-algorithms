...
int getHeight(int in[], int level[], int start, int end, int& height, int n) 
{
    ...
    // Declare two arrays for left and right subtrees 
    ...
    if (leftCount > 0) 
        lheight = getHeight(in, newLeftLevel, start, getIndex - 1, height, leftCount); 
    // Recursively call to calculate height of right Subtree 
    if (rightCount > 0) 
        rheight = getHeight(in, newRightLevel, getIndex + 1, end, height, rightCount); 
    // Current height 
    height = min(lheight + 1, rheight + 1); 
